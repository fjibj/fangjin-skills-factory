...
1. **运行评估**（Node/Fastify 项目）：
   ...
   ```
   自动完成：基线门禁 → 逐场景 HTTP 断言（每场景独立全新数据库）→ 结果存档。
   非 Node 项目：用 `python holdout/evaluate.py {slice-name}-v1`（手动模式，需自行接入断言）。

2. **读取结果**：`holdout/results/{slice-name}-v1.json`，汇总通过/失败/MANUAL。

3. **失败分诊（先判性质，再谈修复）**：
   | 性质 | 信号 | 处理 |
   |---|---|---|
   | 考题问题 | 断言与实现语义不符（状态码臆测、漏参数、操作符误用、混入别层操作） | 修考卷，不改实现，新版本重跑 |
   | 实现缺陷 | 实现行为违反 Intent/Expectations 明确语义 | 记录缺陷，发回构建代理修复，重跑 |
   | 工具问题 | 场景误判 MANUAL / 解析失败（BOM/CRLF 毁 front-matter） | 修文件编码，重跑 |

4. **更新状态文件**：评估完成后 Step 6 → completed；有缺陷则记录到 errors 后回到 Step 5。

> 完整分诊流程与实战参照见 `skills/idsd-evaluate.md`。
...
- [ ] 代为拍板的点（产品/机制决策）是否显式标注？——用户需要知道自己放行了什么
...
### 场景自查（写 holdout 场景时逐条对照）
- [ ] 链路能真实走通吗？每一步 setup 都走 API，不依赖内部状态
- [ ] 状态码是查过的吗？创建类端点按项目惯例 201 还是 200？（别臆测——实战 7 个场景挂在 200/201 上）
- [ ] 匹配操作符语义对吗？`$contains` 只支持字符串，数组用 `$any/$all/$none`
- [ ] 场景只测本切片的数据面吗？别把别的层的操作混进来
- [ ] 隔离/边界语义专门出题了吗？（聚合边界最容易漏实现缺陷）
- [ ] 文件 UTF-8 无 BOM、LF 行尾？（批量改场景禁用会引入 BOM/CRLF 的写回）
- [ ] 无法用断言表达的（UI 呈现、宕机、人工流程）→ `"checks": []` 标 MANUAL

> 完整清单见 `skills/idsd-evaluate.md` 的"出题自查清单"。
